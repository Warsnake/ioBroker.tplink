...
var setParamTypes = function setParamTypes(params, types) {
  if (params && params.length > 0 && types && types.length > 0) {
    return (0, _from2.default)(params).map(function (el, i) {
      if (types[i] === 'number') {
        return +el;
      }
      return el;
...
['getSysInfo', 'getInfo', 'setAlias', 'getModel', { fnName: 'setLocation', paramTypes: ['number', 'number'] }, { fnName: 'reboot', paramTypes: ['number'] }, { fnName: 'reset', paramTypes: ['number'] }].forEach(function (command) {
